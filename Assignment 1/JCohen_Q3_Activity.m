...
% Load training data
X_train = load(fullfile(data_dir, 'train', 'X_train.txt'));
y_train = load(fullfile(data_dir, 'train', 'y_train.txt'));
X_test = load(fullfile(data_dir, 'test', 'X_test.txt'));
y_test = load(fullfile(data_dir, 'test', 'y_test.txt'));
X = [X_train; X_test]; %Combine the dataset
true_labels = [y_train; y_test];
...
fprintf('Classes: %s (1=WALKING, 2=WALKING_UPSTAIRS, 3=WALKING_DOWNSTAIRS, 4=SITTING, 5=STANDING, 6=LAYING)\n', mat2str(classes'));
...
% Perform PCA
fprintf('\nPerforming PCA (this may take a moment with 561 features)...\n');
...
for class = 1:n_classes
    ...
    legend_entries{end+1} = sprintf('%d:%s', classes(class), activity_names{class});
end
...
for class = 1:n_classes
    ...
    legend_entries_3d{end+1} = sprintf('%d:%s', classes(class), activity_names{class});
end
