...
if norm(lambda-[0,1;1,0])<eps % Using 0-1 loss indicates intent to minimize P(error)
    Perror_LDA = [p10LDA,p01LDA]*Nc'/N, % probability of error, empirically estimated
end

disp('Notice that MAP classifier (ERM with 0-1 loss) achieves minimum P(error).');
disp('LDA has a higher error probability than the MAP classifier.');